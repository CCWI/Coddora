...
# CO2 min/max values used for normalization:
min_co2 = 300
max_co2 = 5000 
...
def prepare_data(x, y, x_src=[], y_src=[], 
                 window_size=15, max_batch_size=32, splitAt=None, normalize=True, verbose=1):
    '''
    ...
    normalize        chose one of the following options:
                     - False (default): no normalization
                     - "MinMax": apply MinMax normalization using min and max values of the provided dataset
                     - "CO2":    apply domain-specific normalization for CO2 values (range 300-10000)
    '''
    ...
    # reshape y
    y_train = y_train[:,-1:].reshape(y_train.shape[0], 1) # use last occupancy state in the sequence as the label
    if verbose > 0:
    ...
    if normalize != False:
        ...
        if normalize == 'MinMax':
            ...
            x_train = scale_data(x_train.reshape(-1, 1), min_co2, max_co2)
        ...
        if splitMode.value > 0:
            ...
            if normalize == 'MinMax':
                ...
                x_test = scale_data(x_test.reshape(-1, 1), min_co2, max_co2)
        ...
        if splitMode.value > 1:
            ...
            if normalize == 'MinMax':
                ...
                x_val = scale_data(x_val.reshape(-1, 1), min_co2, max_co2)
